...
export default function App() {
    ...
    const CurrentView = () => {
        ...
        switch (view) {
            ...
            default: return <ApplicationList applications={applications} isLoading={isLoading} onSelectApp={id => handleSetView('detail', id)} />;
        }
    };
...
// --- View Components ---
const ApplicationList = ({ applications, isLoading, onSelectApp }) => {
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {applications.length === 0 ? (<tr><td colSpan="4" className="text-center py-12"><div className="text-lg font-medium text-gray-600">No applications yet!</div></td></tr>) : (
                                applications.map(app => (
                                    <tr key={app.id} onClick={() => onSelectApp(app.id)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm font-medium text-gray-900">{app.company}</div><div className="text-sm text-gray-500">{app.source}</div></td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{app.title}</td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{app.deadline ? new Date(app.deadline).toLocaleDateString() : 'N/A'}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
